...
for task in all_tasks:
    ...
    try:
        ...
        report_labels = sorted(np.unique(np.concatenate([y_te, y_pred])))
        if task == "ROOT_PLANT_SPECIES":
        ...
        cm = confusion_matrix(y_te, y_pred, labels=report_labels)
        ...
        # Save Model
        depth_label = str(best_max_depth) if best_max_depth is not None else "full"
        model_save_path = task_dir / f"model_{task_label.lower()}_n{best_n_estimators}_depth{depth_label}.pkl"
        joblib.dump(model, model_save_path)

        # ========== 6. SAVE REPORT ==========
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        report_filename = f"Report_{task_label}_n{best_n_estimators}_depth{depth_label}.txt"
        with open(task_dir / report_filename, "w", encoding="utf-8") as f:
            f.write(f"TASK: {task_label} | n_estimators: {best_n_estimators} | max_depth: {depth_label}\n")
